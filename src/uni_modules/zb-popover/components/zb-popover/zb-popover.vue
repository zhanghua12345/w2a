  ...
  methods: {
    ...
    getPosition() {
      return new Promise((resolve) => {
        this.$nextTick(() => {
          setTimeout(() => {
            let selectorQuery = uni
              .createSelectorQuery()
              .in(this)
              .selectAll(".zb-button-popover,.zb-popover");
            selectorQuery
                ...
                switch (this.placement) {
                  case "top":
                    if (popoverClientRect.width > width) {
                      popoverStyle.left = `-${
                        (popoverClientRect.width - width) / 2
                      }px`;
                    } else {
                      popoverStyle.left = `${
                        Math.abs(popoverClientRect.width - width) / 2
                      }px`;
                    }
                  ...
                  case "bottom-end":
                    popoverStyle.top = `${height + 8}px`;
                    popoverStyle.right = `0px`;
                    arrowOldStyle.right = "8px";
                    break;
                  case "right":
                    popoverStyle.left = `${width + 8}px`;
                    if (popoverClientRect.height > height) {
                      popoverStyle.top = `-${
                        (popoverClientRect.height - height) / 2
                      }px`;
                    } else {
                      popoverStyle.top = `${Math.abs(
                        (popoverClientRect.height - height) / 2
                      )}px`;
                    }
